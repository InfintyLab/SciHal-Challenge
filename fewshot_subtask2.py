...
def load_fewshot_examples():
    ...
    for domain_name, file_name in domains.items():
        try:
            with open(f"fewshot_example/subtask2/{file_name}.json", "r") as f:
                data = json.load(f)
...
def create_fewshot_prompt_task2(example):
    """Create few-shot prompt with examples."""
    return (
        "You are an assistant for claim verification.\n"
        "Given a claim and some reference from academic paper, "
        "please classify the claim into eight labels: unrelated and unverifiable; entailment; related but unverifiable; misinterpretation; missing information; numeric error; negation; entity error.\n"
...
def train(task, 
    ...
    for idx, row in enumerate(data):
        ...
        system_prompt = create_fewshot_prompt_task2(example)
...
if __name__ == "__main__":
    ...
    # Data loading
    if data_type == "train":
        data_file = f"data/topic/{task}_{data_type}_{batch}_topic.json"  
    else:
